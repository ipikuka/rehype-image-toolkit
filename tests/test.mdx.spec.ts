...
describe("reyhpe-image-hack, with markdown sources", () => {
  ...
  // ******************************************
  it("handle the implicitFigure option for <img> html syntax", async () => {
    const input = dedent`
      <p><img src="image.png" alt="&hello"/></p>
      ...
      <p><a href="http://example.com"><img src="(image.png)" alt="hello"/></a></p>
    `;
  ...
  it("additional properties in html elements <img>, <video> and <audio>", async () => {
    const input = dedent`
      ...
      <video src="image.png" className="ex1" title="title > .new"/>
      <audio src="image.png" className="ex1 ex2" title="title > .new"/>
    `;
  ...
  // ******************************************
  it("disable features ExplicitFigure and ExplicitAutolink", async () => {
    const input = dedent`
      ![^alt]([image.png])
    ...
    const html = String(
      await processMdx(input, "mdx", { explicitAutolink: false, explicitFigure: false }),
    );
